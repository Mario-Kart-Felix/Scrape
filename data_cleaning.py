...
def gaFunc():
  df = pd.read_csv(f"/Users/danielschwartz/Desktop/simplefuture/State_Data/ga/ga.csv", low_memory=False, error_bad_lines=False)    
  df.columns = ['owner_zip', 'owner_name', 'owner_city', 'owner_state']
  df['owner_firstname'] = df['owner_name'].str.split().str[1]
  df['owner_lastname'] = df['owner_name'].str.split().str[0]
  df['owner_middlename'] = df['owner_name'].str.split().str[2]
  df['owner_firstname'].replace('', np.nan, inplace=True)
  ...
  df.dropna(subset=['owner_lastname'], inplace=True)
  df.dropna(subset=['owner_city'], inplace=True)
  df.drop(['owner_name'], axis=1)
  df['reported_tangible_items'] = np.nan
  df['no_of_owners'] = np.nan
  df['cusip'] = np.nan
  df['owner_street_1'] = np.nan 
  df['owner_street_2'] = np.nan 
  df['owner_street_3'] = np.nan 
  df['owner_country_code'] = np.nan   
  df['current_cash_balance'] = np.nan
  ...
  df['holder_zip'] = np.nan
  df['shares_reported'] = np.nan
  df['property_id'] = np.nan
  df['cash_reported'] = np.nan
  df['holder_name'] = np.nan

  df = df[['owner_firstname', 'owner_middlename', 'owner_lastname', 'owner_street_1', 'owner_street_2', 'owner_street_3', 'owner_city', 'owner_state', 'owner_zip', 'no_of_owners', 'owner_country_code', 'current_cash_balance', 'number_of_pending_claims', 'number_of_paid_claims', 'date_of_last_contact', 'holder_name', 'holder_street_1', 'holder_street_2', 'holder_street_3', 'holder_city', 'holder_state', 'holder_zip', 'property_id', 'date_reported', 'property_type', 'cash_reported', 'shares_reported', 'reported_tangible_items', 'cusip']]
  df = df.applymap(lambda s:s.lower() if type(s) == str else s)
  df.to_csv('/Users/danielschwartz/Desktop/Simplefuture/State_Data/ga/new_ga.csv', index=None)

def laFunc():
  # for filename in (os.listdir('/Users/danielschwartz/Desktop/simplefuture/State_Data/LA')):
  df = pd.read_csv(f"/Users/danielschwartz/Desktop/simplefuture/State_Data/LA/LA_9.csv", low_memory=False, error_bad_lines=False)
  df.columns = ['property_id', 'owner_name', 'owner_street_1', 'owner_city', 'owner_state', 'owner_zip', 'property_type', 'cash_reported']
...
def concatFunc(): 
  ...
  for folder in (os.listdir('/Users/danielschwartz/Desktop/simplefuture/state_data')):
    if folder != "concat" and folder != "CA" and folder != "TX":
      print("folder name: " + folder)
      path = os.path.join('/Users/danielschwartz/Desktop/simplefuture/state_data', folder)
      for filename in os.listdir(path):
        if filename.split('_')[0] == "new":
...
def testFunc():
  ...
  #     print("folder name: " + folder)
      # path = os.path.join('/Users/danielschwartz/Desktop/simplefuture/state_data', folder)
      path = '/Users/danielschwartz/Desktop/simplefuture/state_data/ny'
      for filename in os.listdir(path):
        if filename.split('_')[0] == "new":
          frame = os.path.join(path, filename)
          frame = pd.read_csv(frame)
